...
def _v3_draft() -> board_seat_daily.BoardSeatDraft:
...
def test_v3_message_structure_uses_labeled_lines_not_bullets() -> None:
    message = board_seat_daily._render_board_seat_message(company="Anduril", draft=_v3_draft())
...
def test_v3_thesis_contains_four_fixed_labels() -> None:
    message = board_seat_daily._render_board_seat_message(company="Anduril", draft=_v3_draft())
    assert "*Why now:*" in message
    assert "*What's different:*" in message
    assert "*MOS/risks:*" in message
    assert "*Bottom line:*" in message


def test_v3_context_and_funding_use_labeled_lines() -> None:
    message = board_seat_daily._render_board_seat_message(company="Anduril", draft=_v3_draft())
    assert "*Current efforts:*" in message
...
def test_extract_investment_text_parses_v3_labeled_lines() -> None:
    ...
    assert "defense demand is accelerating" in extracted.lower()
    assert "series f" in extracted.lower()
    assert "defense demand is accelerating" in core.lower()
...
def test_repeat_guardrail_still_blocks_duplicate_without_significant_change_v3(tmp_path: Path, monkeypatch) -> None:
    ...
    store = board_seat_daily.BoardSeatStore()
    prior_message = board_seat_daily._render_board_seat_message(company="Anduril", draft=_v3_draft())
    _seed_prior_pitch(
        store=store,
    ...
    monkeypatch.setattr(board_seat_daily, "_llm_draft", lambda **kwargs: _v3_draft())
    monkeypatch.setattr(board_seat_daily, "_build_novel_fallback_draft", lambda **kwargs: _v3_draft())
...
def test_repeat_guardrail_allows_post_with_significant_change_v3(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("COATUE_CLAW_DATA_ROOT", str(tmp_path))
    monkeypatch.setenv("COATUE_CLAW_BOARD_SEAT_DB_PATH", str(tmp_path / "db/board.sqlite"))
    monkeypatch.setenv("COATUE_CLAW_BOARD_SEAT_TZ", "UTC")
    monkeypatch.setenv("COATUE_CLAW_BOARD_SEAT_PORTCOS", "Anduril:anduril")
...
def test_line_word_caps_enforced_for_v3_fields() -> None:
    ...
    clean = board_seat_daily._sanitize_draft(company="Anduril", draft=draft, funding=_funding())
    fields = [
        clean.why_now,
...
def test_run_once_dry_run_v3_contains_hierarchy_sections(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("COATUE_CLAW_DATA_ROOT", str(tmp_path))
    monkeypatch.setenv("COATUE_CLAW_BOARD_SEAT_DB_PATH", str(tmp_path / "db/board.sqlite"))
