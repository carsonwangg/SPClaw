...
from spclaw.hf_youtube_transcript import (
    TranscriptSegment,
...
def test_fetch_youtube_transcript_captions_path(monkeypatch) -> None:
    monkeypatch.setattr(
        "spclaw.hf_youtube_transcript._fetch_video_metadata",
        lambda url, video_id: ("Title", "Channel", 123),
    )
    monkeypatch.setattr(
        "spclaw.hf_youtube_transcript._captions_transcript",
        lambda video_id: [
...
def test_fetch_youtube_transcript_asr_fallback(monkeypatch) -> None:
    ...
    monkeypatch.setattr("spclaw.hf_youtube_transcript._captions_transcript", _raise)
    monkeypatch.setattr(
        "spclaw.hf_youtube_transcript._asr_transcript",
...
def test_fetch_youtube_transcript_raises_when_both_paths_fail(monkeypatch) -> None:
    ...
    monkeypatch.setattr("spclaw.hf_youtube_transcript._captions_transcript", _raise_caps)
    monkeypatch.setattr("spclaw.hf_youtube_transcript._asr_transcript", _raise_asr)
...
def test_asr_transcript_retries_without_response_format_on_incompatible_model(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr("spclaw.hf_youtube_transcript.OpenAI", _Client)
    monkeypatch.setattr("spclaw.hf_youtube_transcript._download_audio", lambda url, tmp_dir: audio_path)
