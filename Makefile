...
openclaw-schedulers-status:
	/opt/spclaw/.venv/bin/python -m spclaw.launchd_runtime status
...
openclaw-memory-status:
	/opt/spclaw/.venv/bin/python -m spclaw.cli memory status

openclaw-memory-prune:
	/opt/spclaw/.venv/bin/python -m spclaw.cli memory prune

openclaw-memory-extract-daily:
	/opt/spclaw/.venv/bin/python -m spclaw.cli memory extract-daily --days $(or $(DAYS),14)

openclaw-memory-reconcile-status:
	/opt/spclaw/.venv/bin/python -m spclaw.cli memory reconcile-status

openclaw-memory-reconcile-export:
	/opt/spclaw/.venv/bin/python -m spclaw.cli memory reconcile-export --limit $(or $(LIMIT),200)

openclaw-files-init:
	/opt/spclaw/.venv/bin/python -m spclaw.file_bridge init-layout

openclaw-files-status:
	/opt/spclaw/.venv/bin/python -m spclaw.file_bridge status

openclaw-files-index:
	/opt/spclaw/.venv/bin/python -m spclaw.file_bridge index

openclaw-files-sync-pull:
	/opt/spclaw/.venv/bin/python -m spclaw.file_bridge pull

openclaw-files-sync-push:
	/opt/spclaw/.venv/bin/python -m spclaw.file_bridge push

openclaw-files-sync:
	/opt/spclaw/.venv/bin/python -m spclaw.file_bridge sync

openclaw-email-status:
	/opt/spclaw/.venv/bin/python -m spclaw.email_gateway status

openclaw-email-run-once:
	/opt/spclaw/.venv/bin/python -m spclaw.email_gateway run-once

openclaw-email-serve:
	/opt/spclaw/.venv/bin/python -m spclaw.email_gateway serve

openclaw-x-chart-status:
	/opt/spclaw/.venv/bin/python -m spclaw.x_chart_daily status

openclaw-x-chart-run-once:
	/opt/spclaw/.venv/bin/python -m spclaw.x_chart_daily run-once --manual

openclaw-x-chart-sources:
	/opt/spclaw/.venv/bin/python -m spclaw.x_chart_daily list-sources --limit $(or $(LIMIT),50)
...
openclaw-spencer-digest-run-once:
	/opt/spclaw/.venv/bin/python -m spclaw.spencer_change_digest run-once $(if $(FORCE),--force,)

openclaw-board-seat-status:
	/opt/spclaw/.venv/bin/python -m spclaw.board_seat_daily status

openclaw-board-seat-run-once:
	/opt/spclaw/.venv/bin/python -m spclaw.board_seat_daily run-once $(if $(FORCE),--force,) $(if $(DRY_RUN),--dry-run,)

openclaw-board-seat-refresh-funding:
	/opt/spclaw/.venv/bin/python -m spclaw.board_seat_daily refresh-funding --all-portcos

openclaw-board-seat-funding-report:
	/opt/spclaw/.venv/bin/python -m spclaw.board_seat_daily funding-quality-report --all-portcos

openclaw-market-daily-status:
	/opt/spclaw/.venv/bin/python -m spclaw.market_daily status
...
openclaw-24x7-enable:
	/opt/spclaw/.venv/bin/python -m spclaw.launchd_runtime enable

openclaw-24x7-status:
	/opt/spclaw/.venv/bin/python -m spclaw.launchd_runtime status

openclaw-24x7-disable:
	/opt/spclaw/.venv/bin/python -m spclaw.launchd_runtime disable --remove-plists

valuation-chart:
	/opt/spclaw/.venv/bin/python -m spclaw.cli valuation-chart $(TICKERS)
